...
writer.close()
print("【注意】：2023-ISCC-练武题排行榜（排名顺序）.xlsx 已生成")
# 按照ID的方式保存为xlsx
df = pd.DataFrame(rlist)
# 将排名列转换为数字类型
...
df.to_excel(writer, index=False, sheet_name="Teams")
writer.close()
print("【注意】：2023-ISCC-练武题排行榜（ID顺序）.xlsx 已生成")
...
df = df.sort_values("排名")
# 将 DataFrame 写入 Excel 文件
writer = pd.ExcelWriter("2023-ISCC-擂台赛排行榜（排名顺序）.xlsx", engine="xlsxwriter")
df.to_excel(writer, index=False, sheet_name="Teams")
writer.close()
print("【注意】：2023-ISCC-擂台赛排行榜（排名顺序）.xlsx 已生成")
# 按照ID的方式保存为xlsx
df = pd.DataFrame(r1list)
# 将排名列转换为数字类型
df["ID"] = pd.to_numeric(df["ID"])
...
df = df.sort_values("ID")
# 将 DataFrame 写入 Excel 文件
writer = pd.ExcelWriter("2023-ISCC-擂台赛排行榜（ID顺序）.xlsx", engine="xlsxwriter")
df.to_excel(writer, index=False, sheet_name="Teams")
writer.close()
print("【注意】：2023-ISCC-擂台赛排行榜（ID顺序）.xlsx 已生成")
print("【注意】：已全部完成！")